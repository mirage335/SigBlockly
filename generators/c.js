 ...
 * Blockly.c.provideFunction_.  It must not be legal code that could
 * legitimately appear in a function definition (or comment), and it must
 * not confuse the regular expression parser.
 */
Blockly.c.FUNCTION_NAME_PLACEHOLDER_ = '{{{}}}';
Blockly.c.FUNCTION_NAME_PLACEHOLDER_REGEXP_ =
    new RegExp(Blockly.c.FUNCTION_NAME_PLACEHOLDER_, 'g');
...
Blockly.c.init = function() {
  ...
  if (Blockly.Variables) {
    ...
    for (var x = 0; x < variables.length; x++) {
      defvars[x] = Blockly.c.variableDB_.getName(variables[x],
 ...
 */
//Disabled by mirage335. For power, we want explicit declarations.
 ...
 *
 * The code gets output when Blockly.c.finish() is called.
 *
 * @param {string} desiredName The desired name of the function (e.g., isPrime).
 * @param {code} A list of c statements.
 * @return {string} The actual name of the new function.  This may differ
 *     from desiredName if the former has already been taken by the user.
 * @private
 */
Blockly.c.provideFunction_ = function(desiredName, code) {
  if (!Blockly.c.definitions_[desiredName]) {
    var functionName = Blockly.c.variableDB_.getDistinctName(
        desiredName, Blockly.Generator.NAME_TYPE);
    Blockly.c.functionNames_[desiredName] = functionName;
    Blockly.c.definitions_[desiredName] = code.join('\n').replace(
        Blockly.c.FUNCTION_NAME_PLACEHOLDER_REGEXP_, functionName);
  }
  return Blockly.c.functionNames_[desiredName];
};
